...
import com.nicname.iyeongjun.nanumcar.adapter.recycler.detail.DetailAdapter
import com.nicname.iyeongjun.nanumcar.api.model.theme.Result
import com.nicname.iyeongjun.nanumcar.ui.activities.main.tempLocation
...
class DetailActivity : DaggerAppCompatActivity(), AnkoLogger {
    ...
    fun setThemeView(item: Result) {
        GlideApp
                ...
                .load(item?.image)
                .into(imgDe)
        txtDeTheme.text = item?.title
        txtDeInfo.text = item?.information
        txtDeTitle.text = item?.dcName
        txtDeSub.text = item.location
    }

    fun setBranchView(item: List<com.nicname.iyeongjun.nanumcar.api.model.branch.Result>) {
...
//                        detailMapview.addTMapPolyLine("Line1", tMapPolyLine)
                        txtDeDistance.text = tMapPolyLine?.distance.toString()
                        detailMapview.apply {
                            setCenterPoint(
